public static class MechConstants
{
    public const float DISTANCE_TO_PATCH = 1.5f;
    public const float MAX_DISTANCE_FOR_USING_ITEM = 3f;
    public const float MAX_DISTANCE_FOR_FASTEN_PATCH = 1f;

    public const int MIN_TREE_HARVEST = 5;
    public const int MAX_TREE_HARVEST = 11;
    public const int MIN_TREE_LOG = 3;
    public const int MAX_TREE_LOG = 6;
    public const int MAX_COUNT_OF_HARVEST = 5;
    public const float WAIT_TIME_FOR_WATER = 30f;

    public const int MAX_USING_OF_FERTILIZER = 10;

    public const int MAX_DAY_QUEST = 2;
    public const int MAX_PLAYER_AMOUNT_QUEST = 5;

    public const int MAX_ITEM_IN_CELL = 32;
    public const int MAX_ITEMS_IN_INVENTORY = 8;
    public const int MAX_ITEMS_IN_PLAYER = 3;
    public const int MAX_ITEMS_IN_CHEST = 4;

    public const int MAX_LEVEL_OF_BUILDING = 3;
    public const float DICREASE_MULTIPLER_FOR_CRAFT_LEVEL_2 = 0.25f;
    public const float DICREASE_MULTIPLER_FOR_CRAFT_LEVEL_3 = 0.5f;
    public const float DISTANCE_TO_BUILDING = 5f;
    public const float MAX_DISTANCE_FOR_FASTEN_BUILDING = 1.5f;
}
